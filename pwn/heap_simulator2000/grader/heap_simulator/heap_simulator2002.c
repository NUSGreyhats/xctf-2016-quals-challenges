...
#else
#define FLAG_2002 "ok now try with the real one"
...
void xrecv(int fd, void *buf, size_t len) {
   if (recv(fd, buf, len, MSG_WAITALL) != len) exit(1);
     //error("short read");
}
...
void you_win() {
    writeend("Congratulations! The key is " FLAG_2002 "\n");
}

void you_lose() {
    writeend("YOU LOSE. GAME OVER\n");
}
...
void handle_request(void) {
  ...
  sendstr(" **** WELCOME TO THE HEAP SIMULATOR 2002****\n\n");
  while (1) {
