...
void alloc_init()
{
	...
	ALLOC_LOG("alloc_init called heap start = 0x%p, heap end = 0x%p\n", g_heap_start, g_heap_end);
}


void dealloc(void *mem)
{
	ALLOC_LOG("dealloc called on 0x%p\n", mem);
...
void *alloc(long numbytes)
{
	...
	ALLOC_LOG("alloc requesting %ld bytes total\n", numbytes);
	chunk_to_use = 0;
	...
	while (current_chunk)
	{
        ...
        ALLOC_LOG("alloc: current chunk 0x%p (%d bytes)\n", current_chunk, current_cb->size);
		if (current_cb->size >= numbytes)
		{
			...
			ALLOC_LOG("chunk location = 0x%p, chunk size = %d\n", chunk_to_use, current_cb->size);
	...
	if (!chunk_to_use)
	{
		...
		sbrk(numbytes);
		ALLOC_LOG("heap end now at 0x%p\n", g_heap_end);
	...
	ALLOC_LOG("alloc returning 0x%p to user\n", chunk_to_use);
	return chunk_to_use;
}
...
void defrag_heap()
{
	...
	{
		current_cb = (control_block_t *)current_chunk;
        ALLOC_LOG("defrag: current chunk 0x%p\n", current_chunk);
...
void dump_heap()
{
	...
	while ((void *)cb < g_heap_end) {
		printf(GREENSTART);
		printf("chunk_loc=0x%p, sz=0x%x, avail=%d, nextfree=0x%p, prevfree=0x%p", cb,cb->size,cb->available,cb->next_free_chunk, cb->prev_free_chunk);
		printf(GREENEND"\n");
